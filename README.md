...
4. ***averageBigwigs.sh*** - script using deeptools to create ip-input and log2(ip/input) tracks, as well as averaging repeats.

5. ***prepareForDeeptools.R*** - script to create a file with a list of average log2(IP/input) with separate column for sample name to use as input for deeptools. Also it takes the table of consensus peaks output by nf-core/chipseq pipeline and creates bedfiles of peaks present in at least 3 replicates among all H3K9me2 ChIP seq samples, or present in all three N2 replicates.

6. ***H3K9me2_overEM88peaks.sh*** - plotting H3K9me2 signal from different samples over set of consensus peaks using deeptools.
